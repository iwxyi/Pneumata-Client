...
function buildChat(overrides: Record<string, unknown> = {}) {
  return normalizeConversation({
    ...
    scenarioState: {
      ...
      progress: [{ key: 'speeches', label: '发言轮次', value: 0, target: 3 }],
    },
    name: '开放讨论',
    topic: '是否要重构推荐系统',
    style: 'brainstorm',
...
describe('DISCUSSION_ENGINE', () => {
  it('builds open discussion prompt context around a concrete goal', () => {
    ...
    expect(context?.promptPrefix).toContain('open analytical group discussion');
    expect(context?.promptPrefix).toContain('是否要重构推荐系统');
    expect(context?.styleProfile).toBe('analytical_room');
    expect(context?.additionalConstraints?.join('\n')).toContain('materially new distinction');
  });

  it('advances open discussion into synthesis at the target speaking count', async () => {
    const chat = buildChat({
      scenarioState: {
        phase: 'discussion',
        turnOrder: ['analyst-a', 'analyst-b', 'analyst-c'],
        goals: [{ goalId: 'discussion-goal', label: '是否要重构推荐系统', status: 'active', progress: 0 }],
        progress: [{ key: 'speeches', label: '发言轮次', value: 2, target: 3 }],
      },
    });
    ...
    expect(result.chatPatch.scenarioState?.phase).toBe('synthesis');
    expect(result.chatPatch.scenarioState?.progress).toEqual([
      { key: 'speeches', label: '发言轮次', value: 3, target: 3 },
    ]);
  ...
  it('keeps roundtable turn order and moves to the next speaker', async () => {
    const chat = buildChat({
      mode: 'roundtable',
      sessionKind: { topology: 'table', family: 'analysis', scenarioId: 'roundtable-discussion', surfaceProfile: 'text' },
      scenarioState: {
        phase: 'roundtable',
        turnOrder: ['analyst-a', 'analyst-b', 'analyst-c'],
    ...
    expect(context?.promptPrefix).toContain('moderated roundtable discussion');
    ...
    expect(result.chatPatch.scenarioState?.progress).toEqual([
      { key: 'speeches', label: '圆桌发言', value: 2, target: 4 },
    ]);
  ...
  it('runs debate as ordered argument turns with assigned roles', async () => {
    const chat = buildChat({
      mode: 'roundtable',
      sessionKind: { topology: 'table', family: 'analysis', scenarioId: 'debate-arena', surfaceProfile: 'text' },
      scenarioState: {
        phase: 'debate',
        ...
        progress: [{ key: 'speeches', label: '攻防轮次', value: 0, target: 3 }],
      },
    });
    ...
    expect(context?.promptPrefix).toContain('structured debate');
    ...
    expect(result.chatPatch.scenarioState?.progress).toEqual([
      { key: 'speeches', label: '攻防轮次', value: 1, target: 3 },
    ]);
  ...
  it('uses brainstorming constraints and progress labels', async () => {
    const chat = buildChat({
      ...
      scenarioState: {
        ...
        progress: [{ key: 'speeches', label: '点子轮次', value: 1, target: 4 }],
      },
    });
    ...
    expect(result.chatPatch.scenarioState?.progress).toEqual([
      { key: 'speeches', label: '点子轮次', value: 2, target: 4 },
    ]);
    expect(result.runtimeEvents[0]?.eventType).toBe('brainstorm_turn');
  });

  it('uses retrospective constraints and enters synthesis at target', async () => {
    const chat = buildChat({
      sessionKind: { topology: 'group', family: 'analysis', scenarioId: 'retrospective-room', surfaceProfile: 'text' },
      scenarioState: {
        phase: 'retrospective',
        discussionMode: 'retrospective',
        goals: [{ goalId: 'discussion-goal', label: '复盘上次发布延期', status: 'active', progress: 0 }],
        progress: [{ key: 'speeches', label: '复盘轮次', value: 2, target: 3 }],
      },
    });
    ...
    expect(result.chatPatch.scenarioState?.phase).toBe('synthesis');
    expect(result.chatPatch.scenarioState?.progress).toEqual([
      { key: 'speeches', label: '复盘轮次', value: 3, target: 3 },
    ]);
    expect(result.runtimeEvents[0]?.eventType).toBe('discussion_synthesis');
  });
});
