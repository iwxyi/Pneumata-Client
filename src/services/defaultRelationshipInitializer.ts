...
function buildPrompt(params: { createdCharacters: AICharacter[]; allCharacters: AICharacter[]; language: 'zh' | 'en'; scope: DefaultRelationshipScope }) {
  ...
  if (params.language === 'zh') {
    return [
      ...
      '除了情绪亲疏，也要识别有明确依据的结构性关系：上下级与职责归属、师徒/亲属、同僚或阵营、债务/命令、宿敌或政治依附。角色资料或可明确识别的经典身份已经足以说明这种结构时，应输出方向性关系；不要因为仅仅同处一个群聊就编造关系。',
      '只要确认一对成员之间存在可影响互动的关系，relationships 必须分别输出 A→B 和 B→A 两条记录，写清双方各自的不同态度、判断或顾虑；不可把两边的内容合并到其中一条。上下级等单向结构也应分别写出“上级相对下级的位置”与“下级相对上级的位置”。双方共同成立的客观事实则写入 sharedStructure，不写进任一方向的 note 或 structure。',
      'note 要写成可展示的简短关系事实，例如“直属上司，负责裁断其差事”或“同属一方但互相提防”，而不是“普通互动”或空泛情绪词。用 deference 表示让位、服从或不服从，用 attachment 表示牵挂与行动上的在意。',
  ...
  return [
    ...
    'In addition to emotional affinity, identify clearly grounded structural relationships: authority and duty, mentor/family, colleagues or factions, command/obligation, rivalry or political dependence. When profiles or clearly identifiable canonical roles establish this structure, output a directional relationship. Do not invent a relationship merely because two characters share a room.',
    'Once you confirm a relationship that can shape interaction, relationships must contain both A→B and B→A entries, each stating that side’s own attitude, judgment, or concern. Never merge both perspectives into one entry. For hierarchy, also state each side’s directional position separately. Facts objectively shared by both sides belong only in sharedStructure, never in either directional note or structure.',
    'Write note as a concise displayable relationship fact, such as “direct superior who assigns their duty” or “same faction but mutually wary”, never as “ordinary interaction” or a vague emotion. Use deference for yielding/obedience or refusal to yield, and attachment for concern that changes action.',
    ...
    'Return strict JSON: {"relationships":[{"fromName":"A","toName":"B","warmth":0,"competence":0,"trust":0,"threat":0,"attachment":0,"deference":0,"note":"natural-language relationship note","confidence":0.8,"reason":"basis"}],"structure":[{"fromName":"A","toName":"B","kind":"authority","statement":"A is B’s direct superior","confidence":0.9,"reason":"basis"}],"sharedStructure":[{"memberNames":["A","B"],"kind":"kinship","statement":"They are sworn siblings","confidence":0.9,"reason":"basis"}]}. structure is only the first member’s directional position toward the second. sharedStructure is a shared fact for two or more members; if a fact is true of both sides, such as kinship, faction, sworn siblinghood, joint debt, or rivalry, it must go in sharedStructure rather than duplicated reverse structure. Kinds must be authority, duty, kinship, affiliation, rivalry, or obligation.',
