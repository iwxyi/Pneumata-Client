...
messages[] is the authoritative ordered list for one speaker's independent consecutive sends (maximum 5); otherwise use null. Each item has non-empty content and optional mediaDecision. Audio must be the only media in its item. Do not split a sentence mechanically or write another actor. extraMessages is legacy-only and null whenever messages[] is used.${turnPlanRules}
