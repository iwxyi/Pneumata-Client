...
4. Do not use intentionalRepeat=true for accidental template drift. If you are merely falling back into the same opener, explanation scaffold, punctuation habit, or generic answer shape, set false and rewrite with a different discourse move.`;
  ...
  const aiDirectInteractionRules = params.chat.type === 'ai_direct'
    ? `\n8. This is an AI private thread with exactly one counterpart in the member list. Unless the turn is truly only self-talk, logistics, or non-directed narration, judge the relationship effect toward that counterpart and return interactionHints.primary with that counterpart's id. Do not leave interactionHints null just because there are only two participants.`
    : '';
